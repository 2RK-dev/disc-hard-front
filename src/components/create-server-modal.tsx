...
interface CreateServerModalProps {
  isOpen: boolean
  onClose: () => void
  onCreateServer: (server: { name: string; description: string; initial: string }) => void
}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="bg-[#313338] text-white border-none sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle className="text-center text-2xl font-bold">Créer un serveur</DialogTitle>
          <DialogDescription className="text-center text-gray-400">
            Votre serveur est un endroit où vous et vos amis pouvez vous retrouver. Créez le vôtre et commencez à
            discuter.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2 sm:gap-0">
            ...
            <Button type="submit" className="bg-[#5865f2] hover:bg-[#4752c4]" disabled={!name || isLoading}>
