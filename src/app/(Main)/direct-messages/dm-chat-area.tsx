...
export function DMChatArea({ selectedDM }: DMChatAreaProps) {
	...
	const handleSendMessage = async (e: React.FormEvent) => {
		...
		if (!DMContent || !YourFriends) return;
			...
			<div className="h-12 border-b border-[#1e1f22] shadow-sm flex items-center px-4">
				<div className="relative">
					<Avatar className="h-7 w-7 mr-2">
						<AvatarImage
							src={YourFriends?.user?.avatar || "/placeholder.svg"}
						/>
						<AvatarFallback>{YourFriends?.alias[0]}</AvatarFallback>
					</Avatar>
					<div
						className={`absolute bottom-0 right-1 h-2.5 w-2.5 rounded-full border-2 border-[#313338] ${getStatusColor(
							YourFriends?.user?.status || "offline"
						)}`}></div>
				</div>
				<h3 className="font-bold text-white">{YourFriends?.alias}</h3>
				<span className="text-xs text-gray-400 ml-2">
					({getStatusText(YourFriends?.user?.status || "offline")})
				</span>
					...
					{DMContent.messages.map((msg) => (
						<div
							...
							className={`flex group ${
								msg.author.id !== YourFriends?.id ? "justify-end" : ""
							}`}>
							{msg.author.id === YourFriends?.id && (
								...
								<p
									className={`${
										msg.author.id !== YourFriends?.id
											? "text-white"
											: "text-gray-200"
									} ${msg.author.id !== YourFriends?.id ? "" : "mt-0.5"}`}>
									{msg.textContent}
								</p>
								{msg.author.id !== YourFriends?.id && (
